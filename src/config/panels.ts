...
// ============================================
// IT QA VARIANT (AI, QA & Testing)
// ============================================
const ITQA_PANELS: Record<string, PanelConfig> = {
  map: { name: 'IT QA Map', enabled: true, priority: 1 },
  'live-news': { name: 'IT & QA Headlines', enabled: true, priority: 1 },
  insights: { name: 'AI Insights', enabled: true, priority: 1 },
  ai: { name: 'AI Development', enabled: true, priority: 1 },
  qa: { name: 'QA Updates', enabled: true, priority: 1 },
  testing: { name: 'Testing News', enabled: true, priority: 1 },
  tech: { name: 'IT & Tech', enabled: true, priority: 1 },
  security: { name: 'Security', enabled: true, priority: 2 },
  'world-clock': { name: 'World Clock', enabled: true, priority: 2 },
  monitors: { name: 'My Monitors', enabled: true, priority: 2 },
};

const ITQA_MAP_LAYERS: MapLayers = {
  gpsJamming: false,
  conflicts: false,
  bases: false,
  cables: true,
...
const ITQA_MOBILE_MAP_LAYERS: MapLayers = {
  ...ITQA_MAP_LAYERS,
  cloudRegions: false,
  techHQs: false,
};
...
// ============================================
export const DEFAULT_PANELS = SITE_VARIANT === 'happy' ? HAPPY_PANELS : SITE_VARIANT === 'tech' ? TECH_PANELS : SITE_VARIANT === 'finance' ? FINANCE_PANELS : SITE_VARIANT === 'itqa' ? ITQA_PANELS : FULL_PANELS;
export const DEFAULT_MAP_LAYERS = SITE_VARIANT === 'happy' ? HAPPY_MAP_LAYERS : SITE_VARIANT === 'tech' ? TECH_MAP_LAYERS : SITE_VARIANT === 'finance' ? FINANCE_MAP_LAYERS : SITE_VARIANT === 'itqa' ? ITQA_MAP_LAYERS : FULL_MAP_LAYERS;
export const MOBILE_DEFAULT_MAP_LAYERS = SITE_VARIANT === 'happy' ? HAPPY_MOBILE_MAP_LAYERS : SITE_VARIANT === 'tech' ? TECH_MOBILE_MAP_LAYERS : SITE_VARIANT === 'finance' ? FINANCE_MOBILE_MAP_LAYERS : SITE_VARIANT === 'itqa' ? ITQA_MOBILE_MAP_LAYERS : FULL_MOBILE_MAP_LAYERS;
...
export const PANEL_CATEGORY_MAP: Record<string, { labelKey: string; panelKeys: string[]; variants?: string[] }> = {
  ...
  // IT QA variant
  itqaCore: {
    labelKey: 'header.panelCatItqaCore',
    panelKeys: ['map', 'live-news', 'insights', 'ai', 'qa', 'testing', 'tech', 'security', 'world-clock', 'monitors'],
    variants: ['itqa'],
  },
