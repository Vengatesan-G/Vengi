...
export function buildArticlePrompts(
  ...
  const isItqaVariant = opts.variant === 'itqa';
  const dateContext = `Current date: ${new Date().toISOString().split('T')[0]}.${isTechVariant || isItqaVariant ? '' : ' Provide geopolitical context appropriate for the current date.'}`;
...
You are an expert IT QA Monitor AI specializing in AI developments, quality assurance updates, and new testing methodologies. Summarize the single most important headline in 2 concise sentences MAX (under 60 words total).
Rules:
- Each numbered headline below is a SEPARATE, UNRELATED story
- Pick the ONE most significant headline and summarize ONLY that story (AI, QA, or testing focus)
- NEVER combine or merge facts from different headlines
- Emphasize: breakthroughs in AI, QA/tool updates, or testing innovations
- Lead with the product/technology/methodology name
- No bullet points, no meta-commentary${langInstruction}`;
      userPrompt = `Each headline below is a separate story. Pick the most important ONE for IT/QA/testing and summarize only that story:\n${headlineText}${intelSection}`;
...
You are an IT QA foresight AI. Analyze the most significant AI/QA/testing development in 2 concise sentences MAX (under 60 words total).
Rules:
- Each numbered headline below is a SEPARATE, UNRELATED story
- Pick the ONE most significant story and analyze ONLY that (AI evolution, QA challenges/solutions, testing trends)
- NEVER combine facts from different headlines
- Focus on: AI integration with QA tools, testing LLMs, shift-left testing, CI/CD
- Lead with the insight, no filler`;
      userPrompt = `Each headline is a separate story. What's the key IT/QA/testing trend?\n${headlineText}${intelSection}`;
    } else if (isTechVariant) {
      systemPrompt = `${dateContext}
...
- If the text is already in ${targetLang}, return it as is.`;
    ...
    systemPrompt = isItqaVariant
      ? `${dateContext}\n\nYou are an IT QA Monitor AI. Pick the most important AI/QA/testing headline and summarize it in 2 concise sentences (under 60 words). Each headline is a separate story - NEVER merge facts. Focus on AI developments, QA updates, testing methodologies.${langInstruction}`
      : isTechVariant
